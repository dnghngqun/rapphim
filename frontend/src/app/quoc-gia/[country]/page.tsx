...
export default function CountryPage() {
  ...
  useEffect(() => {
    setLoading(true);
    let url = `${API_BASE}/countries/${country}/movies?page=${page}&limit=24&sort=year`;
  ...
  const handleYearChange = (e: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    setSelectedYear(val);
    router.push(`/quoc-gia/${country}?page=1${val ? `&year=${val}` : ''}`);
  };
  ...
  return (
    <section className="section" style={{ marginTop: 64, minHeight: '60vh' }}>
      <div className="section-header" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '15px' }}>
        <h1 className="section-title">Phim {countryName} ({totalItems} phim)</h1>
        ...
        <>
          ...
          {totalPages > 1 && (
            <div className="pagination">
              {page > 1 && <Link href={`/quoc-gia/${country}?page=${page - 1}${selectedYear ? `&year=${selectedYear}` : ''}`}>← Trước</Link>}
              {Array.from({ length: Math.min(totalPages, 10) }, (_, i) => {
                ...
                return (
                  <Link key={p} href={`/quoc-gia/${country}?page=${p}${selectedYear ? `&year=${selectedYear}` : ''}`} className={p === page ? 'active' : ''}>{p}</Link>
                );
              })}
              {page < totalPages && <Link href={`/quoc-gia/${country}?page=${page + 1}${selectedYear ? `&year=${selectedYear}` : ''}`}>Sau →</Link>}
            </div>
          )}
        </>
      )}
